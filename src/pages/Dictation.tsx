...
const Dictation = () => {
  ...
  const loadVideoId = async (videoId: string, preferEntryId?: string) => {
    setLoading(true);
    try {
      // 1) Nếu video có phụ đề curated nhúng sẵn → dùng luôn, tránh phụ thuộc
      //    edge function (YouTube đang chặn IP của Edge Runtime).
      const curated =
        (preferEntryId && DICTATION_VIDEOS.find((v) => v.id === preferEntryId)) ||
        DICTATION_VIDEOS.find((v) => v.youtubeId === videoId);
                  ...
                  <CardContent className="space-y-4 p-4">
                    ...
                    {(() => {
                      const latinTokens = Array.from(
                        new Set((seg.hanzi.match(/[A-Za-z0-9]+/g) ?? []))
                      );
